...
`;let f=Date.now();const l=document.querySelector("header"),d=document.querySelector("main"),s=document.querySelector("footer");if(!l||!d||!s)throw new Error("Missing required elements.");const M=s.querySelector("form"),v=s.querySelector("#name"),z=s.querySelector("#phone"),L=s.querySelector("#email"),S=s.querySelector("#honeypot-web"),q=s.querySelector("#honeypot-fax"),E=s.querySelector("textarea"),_=s.querySelector("[type='checkbox'"),w=s.querySelectorAll(".validation-msg"),D=s.querySelector("#privacy-policy"),T=s.querySelector("#submit-btn"),O=l.querySelector("#hamburger"),P=l.querySelector("#nav-mobile"),Z=l.querySelectorAll(".main-menu a"),I=l.querySelector("#logo"),N=d.querySelector("#portfolio-btns"),V=d.querySelector("#portfolio-gallery"),$=d.querySelectorAll(".portfolio-gallery-card"),U=d.querySelector("#hero-cta"),B=/^(\+48\s?)?(\d{3}\s?\d{3}\s?\d{3})$/,F=/\S+@\S+\.\S+/,y={web_1:"https://wgalik.github.io/landing-page/",web_2:"https://wgalik.github.io/news-homepage-main_TailwindCSS_TS/",web_3:"https://wgalik.github.io/loopstudios-landing-page-main_Vue-TS-Tailwind/",web_4:"https://wgalik.github.io/ecommerce-product-page-main_TailwindCSS_TypeScript/"};let c=!1,C=!1,j=!1;const p=e=>{if(e===w.length){w.forEach(t=>t.style.display="none"),j=!1;return}w[e].style.display="block",j=!0},G=e=>{if(!e)return;e.preventDefault();const t=Date.now(),n=S?.value.trim(),i=q?.value.trim();if(n||i)return x();const o=v?.value.trim(),r=z?.value.trim(),a=L?.value.trim(),k=E?.value.trim();if(j&&p(w.length),!o||!k)return p(0);if(!r||!B.test(r))return p(1);if(!a||!F.test(a))return p(2);if(!_?.checked)return p(3);if(t-f<2e3)return;const b={honeypot_1:n,honeypot_2:i,name:o,phone:r,email:a,message:k,startTime:f.toString()};H(b),x()},H=async e=>{if(!e)return;const t="https://galik.cba.pl/index.php",n=new FormData;n.append("honeypot_1",e.honeypot_1),n.append("honeypot_2",e.honeypot_2),n.append("name",e.name),n.append("phone",e.phone),n.append("email",e.email),n.append("message",e.message),n.append("startTime",e.startTime),fetch(t,{method:"POST",body:n}).then(i=>i.text()).then(console.log).catch(console.error),W(e.name,e.email)},u=document.createElement("div"),W=(e,t)=>{if(!e||!t)return;u.innerHTML=`
