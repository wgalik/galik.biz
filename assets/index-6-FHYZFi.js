...
`;let w=Date.now();const l=document.querySelector("header"),d=document.querySelector("main"),s=document.querySelector("footer");if(!l||!d||!s)throw new Error("Missing required elements.");const x=s.querySelector("form"),b=s.querySelector("#name"),v=s.querySelector("#phone"),z=s.querySelector("#email"),L=s.querySelector("#honeypot-web"),S=s.querySelector("#honeypot-fax"),q=s.querySelector("textarea"),A=s.querySelector("[type='checkbox'"),_=s.querySelectorAll(".validation-msg"),D=s.querySelector("#privacy-policy"),M=s.querySelector("#submit-btn"),T=l.querySelector("#hamburger"),O=l.querySelector("#nav-mobile"),Z=l.querySelectorAll(".main-menu a"),I=l.querySelector("#logo"),N=d.querySelector("#portfolio-btns"),V=d.querySelector("#portfolio-gallery"),$=d.querySelectorAll(".portfolio-gallery-card"),U=d.querySelector("#hero-cta"),B=/^([0-9]{9})$/,F=/\S+@\S+\.\S+/,y={web_1:"https://wgalik.github.io/landing-page/",web_2:"https://wgalik.github.io/news-homepage-main_TailwindCSS_TS/",web_3:"https://wgalik.github.io/loopstudios-landing-page-main_Vue-TS-Tailwind/",web_4:"https://wgalik.github.io/ecommerce-product-page-main_TailwindCSS_TypeScript/"};let c=!1,j=!1,E=!1;const p=e=>{if(e===-1){_.forEach(t=>t.style.display="none"),E=!1;return}_[e].style.display="block",E=!0},G=e=>{if(!e||(e.preventDefault(),Date.now()-w<2e3))return;const n=L?.value.trim(),i=S?.value.trim();if(n||i)return C();const o=b?.value.trim(),r=v?.value.trim(),a=z?.value.trim(),h=q?.value.trim();if(E&&p(-1),!o||!h)return p(0);if(!r||!B.test(r))return p(1);if(!a||!F.test(a))return p(2);if(!A?.checked)return p(3);const k={honeypot_1:n,honeypot_2:i,name:o,phone:r,email:a,message:h,startTime:w.toString()};H(k),C()},H=async e=>{if(!e)return;const t="https://galik.cba.pl/index.php",n=new FormData;n.append("honeypot_1",e.honeypot_1),n.append("honeypot_2",e.honeypot_2),n.append("name",e.name),n.append("phone",e.phone),n.append("email",e.email),n.append("message",e.message),n.append("startTime",e.startTime),fetch(t,{method:"POST",body:n}).then(i=>i.text()).then(console.log).catch(console.error),W(e.name,e.email)},u=document.createElement("div"),W=(e,t)=>{if(!e||!t)return;u.innerHTML=`
...
<g style="stroke: none; stroke-width: 0; stroke-dasharray: none; stroke-linecap: butt; stroke-linejoin: miter; stroke-miterlimit: 10; fill: none; fill-rule: nonzero; opacity: 1;" transform="translate(1.4065934065934016 1.4065934065934016) scale(2.81 2.81)">
  ...
  ${r}
  </div>`,document.body.appendChild(a),document.body.style.overflow="hidden",a.querySelector("button")?.addEventListener("click",()=>f(a)),window.addEventListener("keydown",k=>{k.key==="Escape"&&f(a)})},K=e=>{if(!e)return;const t=document.createElement("div");t.classList.add("privacy-policy-wrapper"),t.innerHTML=P,document.body.appendChild(t),document.body.style.overflow="hidden",t.querySelector("button")?.addEventListener("click",()=>f(t)),window.addEventListener("keydown",i=>{i.key==="Escape"&&f(t)})};V?.addEventListener("click",Y);U?.addEventListener("click",g);x?.addEventListener("submit",G);T?.addEventListener("click",m);I?.addEventListener("click",e=>{c?m():g(e)});Z.forEach(e=>e.addEventListener("click",g));O?.addEventListener("click",m);N?.addEventListener("click",X);D?.addEventListener("click",K);window.addEventListener("scroll",()=>{c&&m(),l?.classList.toggle("scrolled",window.scrollY>l.offsetHeight)});window.addEventListener("resize",e=>{c?m():g(e)});
